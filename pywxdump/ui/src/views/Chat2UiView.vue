<template>
    <div class="chat-report">
      ...
      <section class="hot-topics">
        ...
        <div class="topics-container">
          <div class="topic-card" v-for="(topic, index) in reportData.sections.hotTopics.items" :key="'topic-'+index">
            <h3>{{ topic.name }}</h3>
            <div class="topic-category">{{ topic.category }}</div>
            <p class="topic-summary">{{ topic.summary }}</p>
            <div class="topic-keywords">
              <span class="keyword" v-for="(keyword, kIndex) in topic.keywords" :key="'keyword-'+kIndex">{{ keyword }}</span>
            </div>
      ...
      <section class="tutorials">
        ...
        <div class="tutorials-container">
          <div class="tutorial-card" v-for="(tutorial, index) in reportData.sections.tutorials.items" :key="'tutorial-'+index">
            ...
            <div class="key-points">
              <h4>要点：</h4>
              <ul>
                <li v-for="(point, pIndex) in tutorial.keyPoints" :key="'point-'+pIndex">{{ point }}</li>
              </ul>
            </div>
            <div class="tutorial-link">
              <a :href="tutorial.url" class="link valid">查看原文: {{ tutorial.domain }}</a>
            </div>
      ...
      <section class="important-messages">
        ...
        <div class="messages-container">
          <div class="message-card" v-for="(message, index) in reportData.sections.importantMessages.items" :key="'message-'+index">
            <div class="message-meta">
              <span class="time">{{ message.time }}</span>
              <span class="sender">{{ message.sender }}</span>
              <span class="message-type">{{ message.type }}</span>
              <span class="priority" :class="'priority-'+message.priority">优先级：{{ message.priority }}</span>
            </div>
      ...
      <section class="interesting-dialogues">
        ...
        <div class="dialogues-container">
          <div class="dialogue-card" v-for="(dialogue, index) in reportData.sections.dialogues.items" :key="'dialogue-'+index">
            <div class="dialogue-type">{{ dialogue.type }}</div>
            <div class="dialogue-content">
              <div class="message" v-for="(msg, mIndex) in dialogue.messages" :key="'msg-'+mIndex">
                <div class="message-meta">
            ...
            <div class="dialogue-topic">相关话题：{{ dialogue.relatedTopic }}</div>
          </div>
        </div>
      </section>
      ...
      <section class="questions-answers">
        ...
        <div class="qa-container">
          <div class="qa-card" v-for="(qa, index) in reportData.sections.qa.items" :key="'qa-'+index">
            <div class="question">
              ...
              <div class="question-tags">
                <span class="tag" v-for="(tag, tIndex) in qa.question.tags" :key="'tag-'+tIndex">{{ tag }}</span>
              </div>
            </div>
            <div class="answers">
              <div class="answer" v-for="(answer, aIndex) in qa.answers" :key="'answer-'+aIndex">
                <div class="answer-meta">
                  <span class="responder">{{ answer.responder }}</span>
                  <span class="time">{{ answer.time }}</span>
                  <span class="accepted-badge" v-if="answer.isAccepted">最佳回答</span>
                </div>
      ...
      <section class="analytics">
        ...
        <div class="heatmap-container">
          <div class="heat-item" v-for="(heat, index) in reportData.sections.analytics.heatmap" :key="'heat-'+index">
            <div class="heat-topic">{{ heat.topic }}</div>
            <div class="heat-percentage">{{ heat.percentage }}%</div>
            <div class="heat-bar">
              <div class="heat-fill" :style="`width: ${heat.percentage}%; background-color: ${heat.color};`"></div>
            </div>
        ...
        <div class="participants-container">
          <div class="participant-item" v-for="(participant, index) in reportData.sections.analytics.chattyRanking" :key="'participant-'+index">
            ...
            <div class="participant-info">
              ...
              <div class="participant-characteristics">
                <span class="characteristic" v-for="(char, cIndex) in participant.characteristics" :key="'char-'+cIndex">{{ char }}</span>
              </div>
              <div class="participant-words">
                <span class="word" v-for="(word, wIndex) in participant.commonWords" :key="'word-'+wIndex">{{ word }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="night-owls-container">
          <div class="night-owl-item">
            ...
            <div class="owl-info">
              <div class="owl-name">{{ reportData.sections.analytics.nightOwl.name }}</div>
              <div class="owl-title">{{ reportData.sections.analytics.nightOwl.title }}</div>
              <div class="owl-time">最晚活跃时间：{{ reportData.sections.analytics.nightOwl.latestTime }}</div>
              <div class="owl-messages">深夜消息数：{{ reportData.sections.analytics.nightOwl.messageCount }}</div>
              <div class="owl-last-message">{{ reportData.sections.analytics.nightOwl.lastMessage }}</div>
      ...
      <section class="word-cloud">
        ...
        <div class="cloud-container">
          ...
          <div class="cloud-legend">
            <div class="legend-item" v-for="(item, index) in reportData.sections.wordCloud.legend" :key="'legend-'+index">
      ...
      <footer>
        ...
        <p>生成时间：<span class="generation-time">{{ reportData.footer.generationTime }}</span></p>
      ...
      return {
        reportData: {
          ...
          "footer": {
            ...
            "disclaimer": "免责声明：本报告内容基于群聊公开讨论，如有不当内容或侵权问题请联系管理员处理。"
          }
        }
      }
    },
    methods: {
      getRandomPosition(max, offset) {
        return Math.random() * (max - offset);
      },
      hoverWord(index) {
        const word = document.querySelectorAll('.cloud-word')[index];
        word.style.transform = `scale(1.1) rotate(${this.reportData.sections.wordCloud.words[index].rotation}deg)`;
        word.style.zIndex = '10';
      },
      unhoverWord(index) {
        const word = document.querySelectorAll('.cloud-word')[index];
        word.style.transform = `scale(1) rotate(${this.reportData.sections.wordCloud.words[index].rotation}deg)`;
        word.style.zIndex = '1';
      }
    },
    mounted() {
      ...
      words.forEach((word, index) => {
        const wordData = this.reportData.sections.wordCloud.words[index];
        const left = this.getRandomPosition(600, word.textContent.length * 10);
        const top = this.getRandomPosition(400, 20);
  ...
  .cloud-wordcloud {
    ...
    border-radius: 50%;
    box-shadow: 
      40px 40px 0 -5px var(--bg-ui-s-tertiary),
